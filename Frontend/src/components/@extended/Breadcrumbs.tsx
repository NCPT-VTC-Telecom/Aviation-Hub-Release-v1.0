...
//utils
import { getBreadcrumbs } from 'utils/breadcrums';
...
const Breadcrumbs = ({
  ...
  const customBreadcrumbs = getBreadcrumbs(location.pathname);
  const intl = useIntl();
  ...
  if (location.pathname === '/dashboard') {
    return null; // Return null when on the dashboard
  }
        ...
        >
          <Grid item>
            <MuiBreadcrumbs aria-label="breadcrumb" maxItems={maxItems || 8} separator={separatorIcon}>
              {customBreadcrumbs.breadcrumbs.map((crumb: JSX.Element, index: number) => {
                const isLast = index === customBreadcrumbs.breadcrumbs.length - 1;
                const url = customBreadcrumbs.urls[index];
                // const IconComponent = index === 0 ? Home3 : null; // Customize icons for each crumb as needed
                ...
                return (
                  <Typography
                    ...
                    to={url}
                    variant="h6"
                    sx={{ textDecoration: 'none', display: 'flex', alignItems: 'center', color: isLast ? '#000' : '#666' }}
                    key={index}
                  >
                    {/* {IconComponent && <IconComponent size="20" style={{ marginRight: '8px' }} />} */}
                    <span style={{ fontWeight: isLast ? '500' : 'normal', fontSize: '16px' }}>{crumb}</span>
                  </Typography>
                );
              })}
            </MuiBreadcrumbs>
          ...
          {title && titleBottom && (
            <Grid item sx={{ mt: card === false ? 0 : 1 }}>
              <Typography variant="h2" sx={{ fontWeight: 700 }}>
                {customBreadcrumbs.breadcrumbs[customBreadcrumbs.breadcrumbs.length - 1]}
              </Typography>
            </Grid>
          )}
        </Grid>
