...
import AddPlan from './AddPackagesAirfare';
...
import { formatDate, useLangUpdate, getOption } from 'utils/handleData';
...
import { bundleAirfareViewConfig } from 'components/ul-config/view-dialog-config';

const BundleAirfareList = () => {
  ...
  const getData = async (pageIndex: number, pageSize: number, searchValue?: string) => {
    try {
      const res = await getAllPlans(pageIndex, pageSize, 'ticket', searchValue);
  ...
  const columns = useMemo(() => {
    ...
    const excludeColumns = ['product_type'];
  ...
  const handleImport = (data: NewBundle[]) => {
    dispatch(importExcel({ type: 'product', typeSupplier: 'airline', data }));
  };

  const handleExport = () => {
    dispatch(exportExcel({ type: 'product', typeSupplier: 'airline' }));
  };

  const handleGetFormat = () => {
    dispatch(getFormatExcel({ type: 'product', typeSupplier: 'airline' }));
  };
    ...
    <MainCard content={false}>
      <ScrollX>
        <GeneralizedTable
          ...
          data={data}
          handleAdd={handleAdd}
          csvFilename="bundle-airline-list.csv"
      ...
      >
        ...
        <ViewDialog title="bundle-info" config={bundleAirfareViewConfig} open={openDialog} onClose={handleCloseView} data={record} />
      </Dialog>
    </MainCard>
  );
};

export default BundleAirfareList;
