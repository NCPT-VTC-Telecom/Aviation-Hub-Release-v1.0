...
export async function getAllTelecommunication(pageIndex: number, pageSize: number, searchValue?: string) {
  try {
...
export async function postAddTelecommunication(newRecord: NewProvider) {
  try {
...
export async function postDeleteTelecommunication(id: string) {
  try {
...
export async function postEditTelecom(id: string, record: NewProvider) {
  try {
    const res = await axios.post(
