...
const TabSession = () => {
  ...
  async function getTotalSession(pageIndex: number, pageSize: number, deviceID: string, productId?: number) {
    try {
      const res = await getSession(deviceID, pageIndex, pageSize, productId);
      if (res.code === 0) {
  ...
  useEffect(() => {
    if (deviceID) {
      getTotalSession(pageIndex, pageSize, deviceID, filters?.plan);
      getOptionProduct();
    }
    if (isReload) {
      setIsReload(false);
  ...
  }, [deviceID, i18n, isReload, pageIndex, pageSize, filters?.plan]);
  ...
  const columns = useMemo(() => {
    ...
    // Filter out the column with accessor 'flight.aircraft.flight_number'
