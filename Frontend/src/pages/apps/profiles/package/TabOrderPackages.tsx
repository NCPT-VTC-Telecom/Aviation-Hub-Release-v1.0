...
const TabOrderPackages = () => {
  ...
  async function getOrderByPackage(pageSize: number, pageIndex: number, productID: number) {
    try {
      ...
      if (res.code === 0) {
        ...
        setDataOrder(formattedDateTime);
      } else {
        setDataOrder([]);
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (params) {
      ...
      getOrderByPackage(pageIndex, pageSize, parseInt(id));
    }
    if (isReload) {
      setIsReload(false);
    }
...
export default TabOrderPackages;
