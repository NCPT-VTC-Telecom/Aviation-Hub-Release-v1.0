...
// model & utils
// import { postAddAircraft, postDeleteAircraft, postEditAircraft } from './model';
...
const ListAircraftPage = () => {
  ...
  const { getDataAircraft, isLoading, totalPages } = useHandleAircraft();

  const getData = async (pageIndex: number, pageSize: number, searchFilter?: string) => {
    const dataAircraft = await getDataAircraft({ page: pageIndex, pageSize, filters: searchFilter });
